        ...
        while not rospy.is_shutdown():
            try:
                ...
                for i in range(8):
                    ...
                    self.arm_group.set_joint_value_target(self.poses0_top[i])
                    self.plan1 = self.arm_group.go()
                    rospy.sleep(2)
                    self.move(self.poses0_top[i], self.poses0_bottom[i], 10)
                    self.arm_group.set_joint_value_target(self.poses0_top[i])
                    self.plan1 = self.arm_group.go()
                    rospy.sleep(1)
                for i in range(8):

                    self.arm_group.set_joint_value_target(self.poses1_top[i])
                    self.plan1 = self.arm_group.go()
                    rospy.sleep(2)
                    self.move(self.poses1_top[i], self.poses1_bottom[i], 10)
                    self.arm_group.set_joint_value_target(self.poses1_top[i])
                    self.plan1 = self.arm_group.go()
                    rospy.sleep(1)
                for i in range(8):

                    self.arm_group.set_joint_value_target(self.poses2_top[i])
                    self.plan1 = self.arm_group.go()
                    rospy.sleep(2)
                    self.move(self.poses2_top[i], self.poses2_bottom[i], 10)
                    self.arm_group.set_joint_value_target(self.poses2_top[i])
                    self.plan1 = self.arm_group.go()
                    rospy.sleep(1)
                    #go to top position first
                    ...
                    #self.arm_group.set_joint_value_target(self.poses0_bottom[i])

                    #self.plan1 = self.arm_group.go()
                    #if (self.joint3_effort >-0.4) and (self.joint2_effort >-0.4):

                    #    self.arm_group.stop()
                    #    self.arm_group.set_joint_value_target(self.poses0_top[i])
                    #    self.plan1 = self.arm_group.go()
                    #    rospy.loginfo("The effort on joints 2 and 3 is: %f, %f", self.joint2_effort, self.joint3_effort)
