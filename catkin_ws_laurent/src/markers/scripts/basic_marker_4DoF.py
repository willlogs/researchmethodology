...
class MarkerBasics(object):
    def callback_raw_imu(self, msg):
        ...
        ## Here due to only having 4DoF we eliminate the Y-component
        ## of acceleration and X and Z-components of angular velocity
        ## This way the orientation will always be in a radial direction
        self.acc = np.array([-msg.az, 0 , msg.ax])
        self.norm_g = self.acc/np.linalg.norm(self.acc)
        self.gyro = np.array([-0, -msg.gy, 0])/360*2*math.pi
    ...
    def __init__(self):
        ...
        self.pose_pub = rospy.Publisher("/sensor_pose", Pose, queue_size = 1)
        ## Subscriptions
    ...
    def init_marker(self, index = 0):
        ...
        ## Initializing a visualization marker of type ARROW
        self.index = 1000
        self.marker = Marker()
        self.marker2 = Marker()
    ...
    def start(self):
        ...
        while not rospy.is_shutdown():
            ...
            ### In this part we calculate the first estimate of the orientation using accelerometer
            self.rotation = tf.transformations.quaternion_from_euler(0, 0, -(self.joint_positions[1]))
            ...
            # Due to different quaternion formats in different libraries
            # E.G.: q = [x, y, z, w] or q = [w, x, y, z]
            ...
            self.rotated_norm = self.qv_mult(self.quat_updated,[-1,0,0])
            self.rotated_norm[1] = -self.rotated_norm[1]
            self.point.point = [trans[0],trans[1], trans[2]]
            self.point.normal = self.rotated_norm
            self.sp.position.x = trans[0]
            self.sp.position.y = trans[1]
            self.sp.position.z = trans[2]
            self.sp.orientation.x = self.quat_updated[3]
            self.sp.orientation.y = self.quat_updated[0]
            self.sp.orientation.z = self.quat_updated[1]
            self.sp.orientation.w = self.quat_updated[2]
            if self.distance >= 0.001 and self.obstacle:
                ...
                self.marker2.pose.orientation.x = self.quat_updated[3]
                self.marker2.pose.orientation.y = self.quat_updated[0]
                self.marker2.pose.orientation.z = self.quat_updated[1]
                self.marker2.pose.orientation.w = self.quat_updated[2]
                self.index += 1
            ...
            self.marker.pose.orientation.x = self.quat_updated[3]
            self.marker.pose.orientation.y = self.quat_updated[0]
            self.marker.pose.orientation.z = self.quat_updated[1]
            self.marker.pose.orientation.w = self.quat_updated[2]
